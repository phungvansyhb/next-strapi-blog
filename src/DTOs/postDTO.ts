...
export function convertRawPostToPost(rawPost: RawPost): Post {
    ...
    return {
        ...
        imageUrl: process.env.NEXT_PUBLIC_SERVER_URL + (rawPost.attributes.cover?.data?.attributes?.formats?.small?.url || rawPost.attributes.cover?.data?.attributes?.url || ''),
        cover: {
...
export function convertRawArticleToArticle(rawPost: RawArticle): Article {
    return {
        ...
        imageUrl: process.env.NEXT_PUBLIC_SERVER_URL + (rawPost.cover.formats.medium?.url || rawPost.cover?.url || ''),
        cover: {
        ...
        related_post: rawPost.related_article.map(item => ({
            ...item, cover: {
                ...item.cover, url: process.env.NEXT_PUBLIC_SERVER_URL +
