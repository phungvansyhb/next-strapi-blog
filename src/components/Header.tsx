...
const NavItems = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
	({ className, ...props }, ref) => {
		...
		const linkClassName =
			'text-sm text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white transition-colors px-5 py-2';
		return (
			<div
				ref={ref}
...
export default function Header() {
	...
	const linkClassName =
		'inline-flex gap-1 items-center text-sm text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white transition-colors px-5 py-2';
	React.useEffect(() => {
		if (isDesktop) {
			setIsOpen(false);
		}
				...
				<div className='flex flex-wrap justify-between md:flex-nowrap md:gap-10'>
					...
					<div className='order-2 hidden w-full flex-col items-center justify-start md:order-none md:flex md:w-auto md:flex-1 md:flex-row'>
						<DropdownMenu>
							<DropdownMenuTrigger>
								<div
									className={cn(linkClassName + ' flex gap-2 items-center', {
										'font-bold underline': pathName === '/lich-am',
									})}>
